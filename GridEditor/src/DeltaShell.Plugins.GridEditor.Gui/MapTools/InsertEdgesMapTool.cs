...
{
    public class InsertEdgesMapTool : MapTool, IGridEditorMapTool, IDisposable
    {
        ...
        public Func<double, double, double, double, int> GetVertexIndex { get; set; }
        ...
        public override void OnMouseDown(Coordinate worldPosition, MouseEventArgs e)
        {
            ...
            int existingVertexIndex = GetVertexIndex(coordinateToAdd.X, coordinateToAdd.Y, 0.0, searchSize);
        ...
        protected void CommitCoordinates(Coordinate[] toArray)
        {
            if (toArray == null || toArray.Length == 0) return;
            if (toArray.Length == 2)
            {
        ...
        public override void OnPaint(Graphics graphics)
        {
            ...
            var searchSize = Math.Sqrt(Map.Envelope.Area) / 100.0;
            if (GetVertexIndex(lastCoordinatesToUse.X, lastCoordinatesToUse.Y, 0.0, searchSize) != -1)
            {
