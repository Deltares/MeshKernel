...
meshkernel::Smoother::Smoother(std::shared_ptr<Mesh2D> mesh) : m_mesh2d(mesh)
{
}
...
void meshkernel::Smoother::ComputeTopologies()
{
    ...
    for (auto n = 0; n < m_mesh2d->GetNumNodes(); n++)
    {
...
void meshkernel::Smoother::ComputeOperators()
{
    ...
    for (auto n = 0; n < m_mesh2d->GetNumNodes(); n++)
    {
        if (m_mesh2d->m_nodesTypes[n] != 1 && m_mesh2d->m_nodesTypes[n] != 2 && m_mesh2d->m_nodesTypes[n] != 3 && m_mesh2d->m_nodesTypes[n] != 4)
        {
            continue;
        }
...
{
    std::vector<std::vector<double>> J(m_mesh2d->GetNumNodes(), std::vector<double>(4, 0));    // Jacobian
    std::vector<std::vector<double>> Ginv(m_mesh2d->GetNumNodes(), std::vector<double>(4, 0)); // Mesh2D monitor matrices

    for (auto n = 0; n < m_mesh2d->GetNumNodes(); n++)
    {
        if (m_mesh2d->m_nodesTypes[n] != 1 && m_mesh2d->m_nodesTypes[n] != 2 && m_mesh2d->m_nodesTypes[n] != 4)
        {
            continue;
        }
    ...
    m_weights.resize(m_mesh2d->GetNumNodes(), std::vector<double>(m_maximumNumConnectedNodes, 0.0));
    ...
    for (auto n = 0; n < m_mesh2d->GetNumNodes(); n++)
    {
        ...
        if (m_mesh2d->m_nodesNumEdges[n] < 2)
            continue;

        // Internal nodes and boundary nodes
        if (m_mesh2d->m_nodesTypes[n] == 1 || m_mesh2d->m_nodesTypes[n] == 2)
        {
...
void meshkernel::Smoother::ComputeOperatorsNode(int currentNode)
{
    ...
    {
        if (m_topologySharedFaces[currentTopology][f] < 0 || m_mesh2d->m_nodesTypes[currentNode] == 3)
        {
            continue;
        }
        ...
        const auto numFaceNodes = m_mesh2d->GetNumFaceEdges(m_topologySharedFaces[currentTopology][f]);

        // the value of xi and eta needs to be estimated at the circumcenters, calculated the contributions of each node
        if (numFaceNodes == 3)
        {
            // for triangular faces
            int nodeIndex = FindIndex(m_mesh2d->m_facesNodes[m_topologySharedFaces[currentTopology][f]], currentNode);
    ...
    for (int f = 0; f < m_numTopologyFaces[currentTopology]; f++)
    {
        auto edgeIndex = m_mesh2d->m_nodesEdges[currentNode][f];
        int otherNode = m_mesh2d->m_edges[edgeIndex].first + m_mesh2d->m_edges[edgeIndex].second - currentNode;
        int leftFace = m_mesh2d->m_edgesFaces[edgeIndex][0];
        ...
        double rightEta = 0.0;
        double alpha_x = 0.0;
        if (m_mesh2d->IsEdgeOnBoundary(edgeIndex))
        {
            // Boundary face
            ...
            for (int i = 0; i < m_numTopologyNodes[currentTopology]; i++)
            {
                ...
                m_leftXFaceCenterCache[f] += m_mesh2d->m_nodes[m_topologyConnectedNodes[currentTopology][i]].x * m_Az[currentTopology][faceLeftIndex][i];
                m_leftYFaceCenterCache[f] += m_mesh2d->m_nodes[m_topologyConnectedNodes[currentTopology][i]].y * m_Az[currentTopology][faceLeftIndex][i];
            }
            ...
            const double xBc = (1.0 - alpha) * m_mesh2d->m_nodes[currentNode].x + alpha * m_mesh2d->m_nodes[otherNode].x;
            const double yBc = (1.0 - alpha) * m_mesh2d->m_nodes[currentNode].y + alpha * m_mesh2d->m_nodes[otherNode].y;
        ...
        else
        {
            ...
            if ((faceLeft != m_mesh2d->m_edgesFaces[edgeIndex][0] && faceLeft != m_mesh2d->m_edgesFaces[edgeIndex][1]) ||
                (faceRight != m_mesh2d->m_edgesFaces[edgeIndex][0] && faceRight != m_mesh2d->m_edgesFaces[edgeIndex][1]))
            {
            ...
            for (int i = 0; i < m_numTopologyNodes[currentTopology]; i++)
            {
                ...
                m_leftXFaceCenterCache[f] += m_mesh2d->m_nodes[m_topologyConnectedNodes[currentTopology][i]].x * m_Az[currentTopology][faceLeftIndex][i];
                m_leftYFaceCenterCache[f] += m_mesh2d->m_nodes[m_topologyConnectedNodes[currentTopology][i]].y * m_Az[currentTopology][faceLeftIndex][i];
                m_rightXFaceCenterCache[f] += m_mesh2d->m_nodes[m_topologyConnectedNodes[currentTopology][i]].x * m_Az[currentTopology][faceRightIndex][i];
                m_rightYFaceCenterCache[f] += m_mesh2d->m_nodes[m_topologyConnectedNodes[currentTopology][i]].y * m_Az[currentTopology][faceRightIndex][i];
            }
        }
    ...
    double volxi = 0.0;
    for (int i = 0; i < m_mesh2d->m_nodesNumEdges[currentNode]; i++)
    {
    ...
    {
        // internal edge
        if (!m_mesh2d->IsEdgeOnBoundary(m_mesh2d->m_nodesEdges[currentNode][f]))
        {
            int rightNode = f - 1;
            if (rightNode < 0)
            {
                rightNode += m_mesh2d->m_nodesNumEdges[currentNode];
            }
            for (int i = 0; i < m_numTopologyNodes[currentTopology]; i++)
            {
    ...
    for (int n = 0; n < m_mesh2d->m_nodesNumEdges[currentNode]; n++)
    {
        for (int i = 0; i < m_numTopologyNodes[currentTopology]; i++)
        {
                                            ...
                                            int numConnectedNodes)
{
    ...
    for (int f = 0; f < numSharedFaces; f++)
    {
        ...
        int faceLeft = m_mesh2d->m_edgesFaces[edgeIndex][0];
        int faceRight = faceLeft;

        if (!m_mesh2d->IsEdgeOnBoundary(edgeIndex))
        {
            faceRight = m_mesh2d->m_edgesFaces[edgeIndex][1];
        }

        //check if it is a rectangular node (not currentNode itself)
        bool isSquare = true;
        for (int e = 0; e < m_mesh2d->m_nodesNumEdges[nextNode]; e++)
        {
            auto edge = m_mesh2d->m_nodesEdges[nextNode][e];
            for (int ff = 0; ff < m_mesh2d->m_edgesNumFaces[edge]; ff++)
            {
                auto face = m_mesh2d->m_edgesFaces[edge][ff];
                if (face != faceLeft && face != faceRight)
                {
                    isSquare = isSquare && m_mesh2d->GetNumFaceEdges(face) == 4;
                }
            }
            if (!isSquare)
            {
        ...
        if (isSquare)
        {
            if (m_mesh2d->m_nodesTypes[nextNode] == 1 || m_mesh2d->m_nodesTypes[nextNode] == 4)
            {
                // Inner node
                numNonStencilQuad = m_mesh2d->m_nodesNumEdges[nextNode] - 2;
                thetaSquare[f + 1] = (2.0 - double(numNonStencilQuad) * 0.5) * M_PI;
            }
            if (m_mesh2d->m_nodesTypes[nextNode] == 2)
            {
                // boundary node
                numNonStencilQuad = m_mesh2d->m_nodesNumEdges[nextNode] - 1 - m_mesh2d->m_edgesNumFaces[edgeIndex];
                thetaSquare[f + 1] = (1.0 - double(numNonStencilQuad) * 0.5) * M_PI;
            }
            if (m_mesh2d->m_nodesTypes[nextNode] == 3)
            {
                //corner node
                thetaSquare[f + 1] = 0.5 * M_PI;
            }

            if (m_sharedFacesCache[f] > 1 && m_mesh2d->GetNumFaceEdges(m_sharedFacesCache[f]) == 4)
            {
                numNonStencilQuad += 1;
            }
            if (m_sharedFacesCache[leftFaceIndex] > 1 && m_mesh2d->GetNumFaceEdges(m_sharedFacesCache[leftFaceIndex]) == 4)
            {
                numNonStencilQuad += 1;
            }
    ...
    for (int f = 0; f < numSharedFaces; f++)
    {
        ...
        // non boundary face
        if (m_mesh2d->GetNumFaceEdges(m_sharedFacesCache[f]) == 4)
        {
            for (int n = 0; n < m_mesh2d->GetNumFaceEdges(m_sharedFacesCache[f]); n++)
            {
    ...
    for (int f = 0; f < numSharedFaces; f++)
    {
        ...
        auto numFaceNodes = m_mesh2d->GetNumFaceEdges(m_sharedFacesCache[f]);
        ...
        if (isSquareFace[f] || numFaceNodes == 4)
        {
            ...
            phi = OptimalEdgeAngle(numFaceNodes, thetaSquare[f + 1], thetaSquare[nextNode], m_mesh2d->IsEdgeOnBoundary(m_mesh2d->m_nodesEdges[currentNode][f]));
            if (numFaceNodes == 3)
            {
    ...
    double factor = 1.0;
    if (m_mesh2d->m_nodesTypes[currentNode] == 2)
    {
        factor = 0.5;
    }

    if (m_mesh2d->m_nodesTypes[currentNode] == 3)
    {
        factor = 0.25;
    }
    ...
    else if (numSharedFaces > 0)
    {
        ...
        m_nodeXErrors.emplace_back(m_mesh2d->m_nodes[currentNode].x);
        m_nodeXErrors.emplace_back(m_mesh2d->m_nodes[currentNode].y);
    ...
    for (int f = 0; f < numSharedFaces; f++)
    {
        ...
        if (m_sharedFacesCache[f] < 0)
        {
            ...
            else if (m_mesh2d->m_nodesTypes[currentNode] == 3)
            {
                dPhi = 1.5 * M_PI;
            }
            else
        ...
        int numFaceNodes = m_mesh2d->GetNumFaceEdges(m_sharedFacesCache[f]);
        ...
        if (isSquareFace[f])
        {
            ...
            dPhi0 = OptimalEdgeAngle(numFaceNodes, thetaSquare[f + 1], thetaSquare[nextNode], m_mesh2d->IsEdgeOnBoundary(m_mesh2d->m_nodesEdges[currentNode][f]));
            if (numFaceNodes == 3)
            {
        ...
        const auto nodeIndex = FindIndex(m_mesh2d->m_facesNodes[m_sharedFacesCache[f]], currentNode);
        ...
        if ((m_faceNodeMappingCache[f][nextNode] - m_faceNodeMappingCache[f][previousNode]) == -1 ||
            (m_faceNodeMappingCache[f][nextNode] - m_faceNodeMappingCache[f][previousNode]) == m_mesh2d->m_nodesNumEdges[currentNode])
        {
            dTheta = -dTheta;
        }
                                              ...
                                              int& numConnectedNodes)
{
    ...
    numSharedFaces = 0;
    numConnectedNodes = 0;
    if (m_mesh2d->m_nodesNumEdges[currentNode] < 2)
    {
        return;
    }
    ...
    for (int e = 0; e < m_mesh2d->m_nodesNumEdges[currentNode]; e++)
    {
        const auto firstEdge = m_mesh2d->m_nodesEdges[currentNode][e];

        int secondEdgeIndex = e + 1;
        if (secondEdgeIndex >= m_mesh2d->m_nodesNumEdges[currentNode])
        {
            secondEdgeIndex = 0;
        }

        const auto secondEdge = m_mesh2d->m_nodesEdges[currentNode][secondEdgeIndex];
        if (m_mesh2d->m_edgesNumFaces[firstEdge] < 1 || m_mesh2d->m_edgesNumFaces[secondEdge] < 1)
        {
            continue;
        }

        // find the face shared by the two edges
        const int firstFace = std::max(std::min(m_mesh2d->m_edgesNumFaces[firstEdge], int(2)), int(1)) - 1;
        const int secondFace = std::max(std::min(m_mesh2d->m_edgesNumFaces[secondEdge], int(2)), int(1)) - 1;

        if (m_mesh2d->m_edgesFaces[firstEdge][0] != newFaceIndex &&
            (m_mesh2d->m_edgesFaces[firstEdge][0] == m_mesh2d->m_edgesFaces[secondEdge][0] ||
             m_mesh2d->m_edgesFaces[firstEdge][0] == m_mesh2d->m_edgesFaces[secondEdge][secondFace]))
        {
            newFaceIndex = m_mesh2d->m_edgesFaces[firstEdge][0];
        }
        else if (m_mesh2d->m_edgesFaces[firstEdge][firstFace] != newFaceIndex &&
                 (m_mesh2d->m_edgesFaces[firstEdge][firstFace] == m_mesh2d->m_edgesFaces[secondEdge][0] ||
                  m_mesh2d->m_edgesFaces[firstEdge][firstFace] == m_mesh2d->m_edgesFaces[secondEdge][secondFace]))
        {
            newFaceIndex = m_mesh2d->m_edgesFaces[firstEdge][firstFace];
        }
        else
        {
        ...
        if (m_mesh2d->m_nodesNumEdges[currentNode] == 2 && e == 1 && m_mesh2d->m_nodesTypes[currentNode] == 3 && m_sharedFacesCache[0] == newFaceIndex)
        {
            newFaceIndex = intMissingValue;
        }
    ...
    for (int e = 0; e < m_mesh2d->m_nodesNumEdges[currentNode]; e++)
    {
        const auto edgeIndex = m_mesh2d->m_nodesEdges[currentNode][e];
        const auto node = m_mesh2d->m_edges[edgeIndex].first + m_mesh2d->m_edges[edgeIndex].second - currentNode;
        connectedNodesIndex++;
    ...
    for (int f = 0; f < numSharedFaces; f++)
    {
        ...
        int faceNodeIndex = 0;
        const auto numFaceNodes = m_mesh2d->GetNumFaceEdges(faceIndex);
        for (int n = 0; n < numFaceNodes; n++)
        {
            if (m_mesh2d->m_facesNodes[faceIndex][n] == currentNode)
            {
                faceNodeIndex = n;
                break;
            }
        ...
        for (int n = 0; n < numFaceNodes; n++)
        {
            ...
            const auto node = m_mesh2d->m_facesNodes[faceIndex][faceNodeIndex];
...
void meshkernel::Smoother::Initialize()
{
    ...
    m_numConnectedNodes.resize(m_mesh2d->GetNumNodes());
    std::fill(m_numConnectedNodes.begin(), m_numConnectedNodes.end(), 0.0);

    m_connectedNodes.resize(m_mesh2d->GetNumNodes());
    ...
    m_nodeTopologyMapping.resize(m_mesh2d->GetNumNodes());
...
void meshkernel::Smoother::ComputeJacobian(int currentNode, std::vector<double>& J) const
{
    ...
    if (m_mesh2d->m_projection == Projection::cartesian)
    {
        J[0] = 0.0;
        J[1] = 0.0;
        J[2] = 0.0;
        ...
        {
            J[0] += m_Jxi[currentTopology][i] * m_mesh2d->m_nodes[m_topologyConnectedNodes[currentTopology][i]].x;
            J[1] += m_Jxi[currentTopology][i] * m_mesh2d->m_nodes[m_topologyConnectedNodes[currentTopology][i]].y;
            J[2] += m_Jeta[currentTopology][i] * m_mesh2d->m_nodes[m_topologyConnectedNodes[currentTopology][i]].x;
            J[3] += m_Jeta[currentTopology][i] * m_mesh2d->m_nodes[m_topologyConnectedNodes[currentTopology][i]].y;
        }
    }
    if (m_mesh2d->m_projection == Projection::spherical || m_mesh2d->m_projection == Projection::sphericalAccurate)
    {
        double cosFactor = std::cos(m_mesh2d->m_nodes[currentNode].y * degrad_hp);
        J[0] = 0.0;
        J[1] = 0.0;
        J[2] = 0.0;
        J[3] = 0.0;
        for (int i = 0; i < numNodes; i++)
        {
            J[0] += m_Jxi[currentTopology][i] * m_mesh2d->m_nodes[m_topologyConnectedNodes[currentTopology][i]].x * cosFactor;
            J[1] += m_Jxi[currentTopology][i] * m_mesh2d->m_nodes[m_topologyConnectedNodes[currentTopology][i]].y;
            J[2] += m_Jeta[currentTopology][i] * m_mesh2d->m_nodes[m_topologyConnectedNodes[currentTopology][i]].x * cosFactor;
            J[3] += m_Jeta[currentTopology][i] * m_mesh2d->m_nodes[m_topologyConnectedNodes[currentTopology][i]].y;
        }
    }
}
