...
namespace GridGeom
{
    ...
    // spline related
    /// these functions are used in the gridgeom api
    /// SPLINE
    /// second order derivative of spline coordinates
    static bool SecondOrderDerivative(const std::vector<GridGeom::Point>& coordinates, int numNodes, std::vector<GridGeom::Point>& coordinatesDerivatives)
    {
        std::vector<GridGeom::Point> u(numNodes);
        u[0] = { 0.0, 0.0 };
        ...
        {
            const GridGeom::Point p = coordinatesDerivatives[i - 1] * 0.5 + 2.0;
            coordinatesDerivatives[i].x = -0.5 / p.x;
            coordinatesDerivatives[i].y = -0.5 / p.y;

            const GridGeom::Point delta = coordinates[i + 1] - coordinates[i] - (coordinates[i] - coordinates[i - 1]);
