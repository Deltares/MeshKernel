...
void meshkernel::CurvilinearGridFromSplinesTransfinite::Compute(CurvilinearGrid& curvilinearGrid)
{
    ...
    {
        throw std::invalid_argument("CurvilinearGridFromSplinesTransfinite::Compute\n There are no rows or no columns.");
    }

    const auto numSplines = m_splines->m_numSplines;
    if (numSplines < 4)
    {
        throw std::invalid_argument("CurvilinearGridFromSplinesTransfinite::Compute\n The number of splines is less than four.");
    }
    ...
    for (int splineIndex = 0; splineIndex < numSplines; splineIndex++)
    {
        ...
        {
            throw std::invalid_argument("CurvilinearGridFromSplinesTransfinite::Compute\n The number of intersections are less than two.");
        }
...
void meshkernel::CurvilinearGridFromSplinesTransfinite::ComputeIntersections()
{
    ...
    for (int i = 0; i < numSplines; i++)
    {
        for (int j = i + 1; j < numSplines; j++)
        {
            ...
            if (areCrossing)
            {
                ...
                {
                    throw std::invalid_argument("CurvilinearGridFromSplinesTransfinite::Compute\n At least two splines are intersecting twice.");
                }
    ...
    for (int i = 0; i < numSplines; i++)
    {
        if (m_splineType[i] == 0)
        {
            throw std::invalid_argument("CurvilinearGridFromSplinesTransfinite::Compute\n At least one of the splines could not be classified.");
        }
    }
