...
TEST(Mesh1D, GenerateMeshFromPolyLines_WithInexactOffset_ShouldGenerateMesh)
{
    ...
    // 2 Execution
    const auto mesh = meshkernel::Mesh1D(polylines, 6.0, 0.01, meshkernel::Projection::cartesian);

    // 3 Assertion
    const auto tolerance = 1e-6;
    ASSERT_NEAR(0.0, mesh.m_nodes[0].x, tolerance);
    ASSERT_NEAR(6.0, mesh.m_nodes[1].x, tolerance);
    ASSERT_NEAR(12.0, mesh.m_nodes[2].x, tolerance);
    ASSERT_NEAR(18.0, mesh.m_nodes[3].x, tolerance);
    ASSERT_NEAR(20.0, mesh.m_nodes[4].x, tolerance);
    ASSERT_NEAR(10.0, mesh.m_nodes[5].x, tolerance);
    ASSERT_NEAR(10.0, mesh.m_nodes[6].x, tolerance);
    ASSERT_NEAR(10.0, mesh.m_nodes[7].x, tolerance);
    ASSERT_NEAR(10.0, mesh.m_nodes[8].x, tolerance);
    ASSERT_NEAR(10.0, mesh.m_nodes[9].x, tolerance);
    ...
    ASSERT_NEAR(-10.0, mesh.m_nodes[5].y, tolerance);
    ASSERT_NEAR(-4.0, mesh.m_nodes[6].y, tolerance);
    ASSERT_NEAR(2.0, mesh.m_nodes[7].y, tolerance);
    ASSERT_NEAR(8.0, mesh.m_nodes[8].y, tolerance);
    ASSERT_NEAR(10.0, mesh.m_nodes[9].y, tolerance);
}