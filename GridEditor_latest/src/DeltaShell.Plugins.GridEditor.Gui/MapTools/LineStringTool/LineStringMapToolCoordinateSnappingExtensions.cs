...
namespace DeltaShell.Plugins.GridEditor.Gui.MapTools.LineStringTool
{
    internal static class LineStringMapToolCoordinateSnappingExtensions
    {
        ...
        public static TrackerFeature SnappedTrackerAtCoordinate(this Coordinate worldPosition, IFeatureInteractor featureInteractor)
        {
            ...
            if (featureInteractor is SplineEditorFeatureInteractor splineEditorFeatureInteractor)
            {
                var spline = splineEditorFeatureInteractor.SourceFeature as Spline;
                int splineIndex = index.Value / spline.PointsBetweenSplineCornerPoints;
                return featureInteractor.Trackers[splineIndex];
            }

            return featureInteractor.Trackers[index.Value];
        }
