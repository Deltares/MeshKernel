...
namespace meshkernel
{
    ...
    UInt Mesh::ConnectNodes(UInt startNode, UInt endNode)
    {
    ...
    UInt Mesh::InsertNode(const Point& newPoint)
    {
    ...
    UInt Mesh::FindCommonNode(UInt firstEdgeIndex, UInt secondEdgeIndex) const
    {
    ...
    UInt Mesh::FindEdge(UInt firstNodeIndex, UInt secondNodeIndex) const
    {
    ...
    UInt Mesh::FindNodeCloseToAPoint(Point const& point, double searchRadius)
    {
        if (GetNumNodes() <= 0)
        {
        ...
        SearchNearestLocation(point, searchRadius * searchRadius, MeshLocation::Nodes);

        if (GetNumLocations(MeshLocation::Nodes) > 0)
        {
            return GetLocationsIndices(0, MeshLocation::Nodes);
        }

        throw AlgorithmError("Could not find the node index close to a point.");
    }

    UInt Mesh::FindNodeCloseToAPoint(Point point, const std::vector<bool>& oneDNodeMask)
    {
        if (GetNumNodes() <= 0)
        {
            throw std::invalid_argument("Mesh::FindNodeCloseToAPoint: There are no valid nodes.");
        ...
        for (UInt index = 0; index < GetNumLocations(MeshLocation::Nodes); ++index)
        {
            const auto nodeIndex = GetLocationsIndices(index, MeshLocation::Nodes);
    ...
    UInt Mesh::FindEdgeCloseToAPoint(Point point)
    {
        if (GetNumEdges() == 0)
        {
        ...
        if (GetNumLocations(MeshLocation::Edges) >= 1)
        {
            return GetLocationsIndices(0, MeshLocation::Edges);
        }
    ...
    std::vector<Point> Mesh::ComputeLocations(MeshLocation::Type meshLocation) const
    {
        ...
        if (meshLocation == MeshLocation::Edges)
        {
        ...
        if (meshLocation == MeshLocation::Faces)
        {
