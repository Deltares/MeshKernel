...
meshkernel::PolygonGroup::PolygonGroup (const std::vector<Point>& points,
...
meshkernel::Polygon meshkernel::PolygonGroup::ConstructPolygon (const std::vector<Point>& points,
                                                                size_t start, size_t end,
                                                                Projection projection)
{
    if (start > end)
...
void meshkernel::PolygonGroup::ConstructOuterPolygon ( const std::vector<Point>& points,
                                                       size_t start, size_t end,
...
void meshkernel::PolygonGroup::ConstructInnerPolygons ( const std::vector<Point>& points,
                                                        const IndexRangeArray& innerIndices,
                                                        Projection projection)
{
    if (innerIndices.size () <= 1)
...
bool meshkernel::PolygonGroup::Contains (const Point& pnt) const
{
...
int meshkernel::PolygonGroup::ContainsRegion (const Point& pnt) const
{
    int result = 0;
...
void meshkernel::PolygonGroup::SnapToLandBoundary(size_t startIndex, size_t endIndex, const LandBoundary& landBoundary)
{
