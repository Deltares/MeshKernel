                                     ...
                                     double projectionFactor)
{
    ...
    for (Index n = 0; n < m_mesh1d->m_nodes.size(); ++n)
    {
...
bool Contacts::IsContactIntersectingMesh1d(Index node,
                                           Index face) const
{
    for (Index e = 0; e < m_mesh1d->GetNumEdges(); ++e)
    {
...
bool Contacts::IsContactIntersectingContact(Index node, Index face) const
{
    for (Index i = 0; i < m_mesh1dIndices.size(); ++i)
    {
        Point intersectionPoint;
        double crossProduct;
        double ratioFirstSegment;
...
void Contacts::ComputeMultipleContacts(const std::vector<bool>& oneDNodeMask)
{
    ...
    for (Index e = 0; e < m_mesh1d->GetNumEdges(); ++e)
    {
        ...
        for (Index f = 0; f < numNeighbours; ++f)
        {
            ...
            for (Index ee = 0; ee < m_mesh2d->m_numFacesNodes[face]; ++ee)
            {
                Point intersectionPoint;
                double crossProduct;
                double ratioFirstSegment;
                                           ...
                                           const Polygons& polygons)
{
    ...
    std::vector<Index> facePolygonIndex(m_mesh2d->GetNumFaces(), constants::missing::sizetValue);
    std::vector<bool> faceInPolygon(m_mesh2d->GetNumFaces(), false);
    for (Index faceIndex = 0; faceIndex < m_mesh2d->GetNumFaces(); ++faceIndex)
    {
    ...
    std::vector<Index> closest1dNodeIndices(polygons.GetNumPolygons(), constants::missing::sizetValue);
    std::vector<Index> closest2dNodeIndices(polygons.GetNumPolygons(), constants::missing::sizetValue);
    for (Index faceIndex = 0; faceIndex < m_mesh2d->GetNumFaces(); ++faceIndex)
    {
    ...
    for (Index polygonIndex = 0; polygonIndex < polygons.GetNumPolygons(); ++polygonIndex)
    {
                                       ...
                                       double searchRadius)
{
    ...
    std::vector<Index> faceTo1DNode(m_mesh2d->GetNumFaces(), constants::missing::sizetValue);
    for (Index n = 0; n < m_mesh1d->GetNumNodes(); ++n)
    {
        ...
        for (Index f = 0; f < faceCircumcentersRTree.GetQueryResultSize(); ++f)
        {
    ...
    for (Index f = 0; f < m_mesh2d->GetNumFaces(); ++f)
    {
