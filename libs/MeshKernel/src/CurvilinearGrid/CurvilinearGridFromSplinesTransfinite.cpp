...
CurvilinearGrid CurvilinearGridFromSplinesTransfinite::Compute()
{
    ...
    Index numMSplines = 0;
    Index numNSplines = 0;
    for (Index splineIndex = 0; splineIndex < numSplines; splineIndex++)
    {
        Index numIntersections = 0;
    ...
    for (Index i = 0; i < numMSplines - 1; i++)
    {
        for (Index j = 0; j < numNSplines - 1; j++)
        {
            // Fill each block of the interpolation plane
            for (Index k = 0; k < numMPoints; k++)
            {
                for (Index l = 0; l < numNPoints; l++)
                {
...
void CurvilinearGridFromSplinesTransfinite::ComputeDiscretizations(Index numIntersections,
                                                                   Index numPoints,
                                                                   Index numDiscretizations,
                                                                   ...
                                                                   std::vector<double>& distances) const
{
    ...
    if (numIntersections > 2)
    {
        ...
        for (Index i = 1; i < numIntersections - 1; i++)
        {
        ...
        for (Index i = 0; i < numIntersections - 1; i++)
        {
            ...
            for (Index j = 0; j < numDiscretizations + 1; j++)
            {
                                                                        ...
                                                                        std::vector<double>& distances) const
{
    ...
    for (Index i = 0; i < distances.size() - 1; i++)
    {
...
void CurvilinearGridFromSplinesTransfinite::ComputeIntersections()
{
    ...
    for (Index i = 0; i < numSplines; i++)
    {
        for (Index j = i + 1; j < numSplines; j++)
        {
    ...
    const Index maxExternalIterations = 10;
    for (Index i = 0; i < maxExternalIterations; i++)
    {
        // sort along m_m
        const Index maxInternalIterations = 100;
        for (Index j = 0; j < maxInternalIterations; j++)
        {
    ...
    ResizeAndFill2DVector(m_splineGroupIndexAndFromToIntersections, numSplines, 3, true, static_cast<Index>(0));

    // m_n direction
    for (Index i = 0; i < m_numMSplines; i++)
    {
    ...
    {
        for (Index j = 0; j < m_numMSplines; j++)
        {
            Index maxIndex = 0;
            Index lastIndex = m_numMSplines;
...
bool CurvilinearGridFromSplinesTransfinite::OrderSplines(Index startFirst,
                                                         Index endFirst,
                                                         Index startSecond,
                                                         Index endSecond)
{
...
template <typename T>
void CurvilinearGridFromSplinesTransfinite::SwapColumns(std::vector<std::vector<T>>& v, Index firstColumn, Index secondColumn) const
{
