...
TEST(MeshRefinement, RefineElongatedFaces)
{
    ...
    ASSERT_NEAR(1660.6851354957175, faceCircumcenters[1].x, tolerance);
    ASSERT_NEAR(1660.5667704694627, faceCircumcenters[2].x, tolerance);
    ASSERT_NEAR(1672.0912775041329, faceCircumcenters[3].x, tolerance);
    ASSERT_NEAR(1659.9354211078053, faceCircumcenters[4].x, tolerance);
    ASSERT_NEAR(1659.8248648846848, faceCircumcenters[5].x, tolerance);
    ASSERT_NEAR(1671.1074693287451, faceCircumcenters[6].x, tolerance);
    ASSERT_NEAR(1659.1859707978906, faceCircumcenters[7].x, tolerance);
    ASSERT_NEAR(1659.0828479935451, faceCircumcenters[8].x, tolerance);
    ASSERT_NEAR(1670.1135380487042, faceCircumcenters[9].x, tolerance);
    ASSERT_NEAR(1658.4375379474418, faceCircumcenters[10].x, tolerance);

    ASSERT_NEAR(645.10565853980427, faceCircumcenters[0].y, tolerance);
    ASSERT_NEAR(646.20173898461292, faceCircumcenters[1].y, tolerance);
    ASSERT_NEAR(654.66978646149676, faceCircumcenters[2].y, tolerance);
    ASSERT_NEAR(662.96936808193914, faceCircumcenters[3].y, tolerance);
    ASSERT_NEAR(664.17198930960728, faceCircumcenters[4].y, tolerance);
    ASSERT_NEAR(672.49588595953537, faceCircumcenters[5].y, tolerance);
    ASSERT_NEAR(680.82566423059006, faceCircumcenters[6].y, tolerance);
    ASSERT_NEAR(682.12720924968505, faceCircumcenters[7].y, tolerance);
    ASSERT_NEAR(690.31918193748425, faceCircumcenters[8].y, tolerance);
    ASSERT_NEAR(698.66471917887850, faceCircumcenters[9].y, tolerance);
    ASSERT_NEAR(700.06356972686194, faceCircumcenters[10].y, tolerance);
...
{
    meshkernel::UInt id;
    meshkernel::UInt startNode;
    meshkernel::UInt endNode;
    meshkernel::UInt leftFace;
    meshkernel::UInt rightFace;
};

struct BoundaryEdge
{
    meshkernel::UInt edgeId;
    meshkernel::UInt neighbourNode;
    meshkernel::UInt leftFace; // Store face mapping on the edge for easy retrieval during loop trace
...
{
    static const UInt nullValue = constants::missing::uintValue;

}

// Main function to extract loops
std::vector<std::vector<meshkernel::Point>> findBoundaryLoops(const std::vector<meshkernel::Point>& nodes,
                                                              const std::vector<Edge2>& edges,
                                                              const std::vector<std::array<meshkernel::UInt, 2>>& edgesFaces [[maybe_unused]])
{

    // // Step 1 & 2: Build a radially sorted boundary adjacency list
    // // Node ID -> list of connected boundary edges
    // std::unordered_map<meshkernel::UInt, std::vector<BoundaryEdge>> boundaryAdjacency;
    // std::unordered_map<meshkernel::UInt, bool> edgeVisited;

    // for (meshkernel::UInt edgeId = 0; const auto& edge : edges)
    // {
    //     if (edgesFaces[edgeId][1] == nullValue)
    //     {
    //         edgeVisited[edgeId] = false;

    //         // Calculate geometric angles for pinch-point sorting
    //         double dx = nodes[edge.second].x - nodes[edge.first].x;
    //         double dy = nodes[edge.second].y - nodes[edge.first].y;

    //         double angle_start_to_end = std::atan2(dy, dx);
    //         double angle_end_to_start = std::atan2(-dy, -dx);

    //         boundaryAdjacency[edge.first].push_back({edgeId, edge.second, angle_start_to_end});
    //         boundaryAdjacency[edge.second].push_back({edgeId, edge.first, angle_end_to_start});
    //     }

    //     ++edgeId;
    // }

    // // Sort outgoing boundary edges at each node counter-clockwise
    // for (auto& [node_id, connected_edges] : boundaryAdjacency)
    // {
    //     std::sort(connected_edges.begin(), connected_edges.end(), [](const BoundaryEdge& a, const BoundaryEdge& b)
    //               { return a.angle < b.angle; });
    // }

    // std::vector<std::vector<meshkernel::UInt>> allLoops;

    // // Step 3: Trace loops
    // for (meshkernel::UInt edgeId = 0; const auto& edge : edges)
    // {
    //     if (edgesFaces[edgeId][1] != nullValue || edgeVisited[edgeId])
    //         continue;

    //     std::vector<meshkernel::UInt> currentLoop;
    //     meshkernel::UInt currentNode = edge.first;
    //     meshkernel::UInt startNode = currentNode;

    //     while (true)
    //     {
    //         currentLoop.push_back(currentNode);

    //         // Find the next unvisited boundary edge radiating from currentNode
    //         auto& options = boundaryAdjacency[currentNode];
    //         meshkernel::UInt nextNode = nullValue;
    //         meshkernel::UInt chosenEdgeId = nullValue;

    //         for (const auto& option : options)
    //         {
    //             if (!edgeVisited[option.edgeId])
    //             {
    //                 chosenEdgeId = option.edgeId;
    //                 nextNode = option.neighbourNode;
    //                 break;
    //             }
    //         }

    //         if (chosenEdgeId == nullValue)
    //             break; // Loop fully completed or dead end reached

    //         edgeVisited[chosenEdgeId] = true;
    ...
    //     {
    //         currentLoop.push_back(currentLoop.front());
    ...
    // 1. Collect all boundary edges (where rightFace is nullValue)
    for (const auto& edge : edges)
    {
        if (edge.rightFace == constants::missing::uintValue)
        {
            edgeVisited[edge.id] = false;

            double dx = nodes[edge.endNode].x - nodes[edge.startNode].x;
            double dy = nodes[edge.endNode].y - nodes[edge.startNode].y;

            // Angular direction of the edge
            double angle_start_to_end = normalizeAngle(std::atan2(dy, dx));
            double angle_end_to_start = normalizeAngle(std::atan2(-dy, -dx));

            boundaryAdjacency[edge.startNode].push_back({edge.id, edge.endNode, 0, angle_start_to_end});
            boundaryAdjacency[edge.endNode].push_back({edge.id, edge.startNode, 0, angle_end_to_start});
        }
    }

    // 2. Sort outgoing edges counter-clockwise around every node
    for (auto& [node_id, connected_edges] : boundaryAdjacency)
    {
        std::sort(connected_edges.begin(), connected_edges.end(), [](const BoundaryEdge& a, const BoundaryEdge& b)
                  { return a.angle < b.angle; });
    ...
    std::vector<std::vector<meshkernel::Point>> allLoops;
    // 3. Trace loops tracking the incoming direction
    for (const auto& edge : edges)
    {
        if (edge.rightFace != constants::missing::uintValue || edgeVisited[edge.id])
            continue;

        std::vector<meshkernel::Point> currentLoop;

        // Start tracing this edge from startNode to endNode
        meshkernel::UInt prevNode = edge.startNode;
        meshkernel::UInt currentNode = edge.endNode;

        currentLoop.push_back(nodes[prevNode]);
        edgeVisited[edge.id] = true;

        // Calculate initial incoming angle into the second node
        double dx = nodes[currentNode].x - nodes[prevNode].x;
        double dy = nodes[currentNode].y - nodes[prevNode].y;
        double incomingAngle = normalizeAngle(std::atan2(dy, dx));

        while (currentNode != edge.startNode)
        {
            currentLoop.push_back(nodes[currentNode]);

            const auto& options = boundaryAdjacency[currentNode];
            double bestDelta = 3.0 * std::numbers::pi; // Higher than max possible delta (2*PI)
            size_t chosenOptionIdx = static_cast<size_t>(-1);

            // Find the boundary edge that tracks closest clockwise to keep empty space on the right
            for (size_t i = 0; i < options.size(); ++i)
            {
                if (edgeVisited[options[i].edgeId])
                    continue;

                double delta = incomingAngle - options[i].angle;
                if (delta < 0)
            ...
            // If no unvisited edges are found, break out safely
            if (chosenOptionIdx == static_cast<size_t>(-1))
                break;

            const auto& chosen = options[chosenOptionIdx];
            edgeVisited[chosen.edgeId] = true;

            // Step forward
            prevNode = currentNode;
            currentNode = chosen.neighbourNode;
            incomingAngle = chosen.angle;
        }
...
// void findBoundaryLoops(const std::vector<meshkernel::Point>& nodes,
//                        const std::vector<Edge2>& edges,
//                        std::vector<std::vector<meshkernel::UInt>>& allLoops, // Out: The boundary node sequences
//                        std::vector<meshkernel::UInt>& firstTouchedFaces)     // Out: The associated face IDs
// {
//     // Clear outputs to ensure clean state
//     allLoops.clear();
//     firstTouchedFaces.clear();
...
//     // 2. Sort outgoing edges counter-clockwise
//     for (auto& [node_id, connected_edges] : boundaryAdjacency)
//     {
//         std::sort(connected_edges.begin(), connected_edges.end(), [](const BoundaryEdge& a, const BoundaryEdge& b)
//                   { return a.angle < b.angle; });
...
//     for (const auto& edge : edges)
//     {
//         if (edge.rightFace != nullValue || edgeVisited[edge.id])
//             continue;

//         // Capture the face ID of the first edge that starts this loop
//         meshkernel::UInt associatedFaceId = edge.leftFace;

//         std::vector<meshkernel::UInt> currentNodes;
//         meshkernel::UInt prevNode = edge.startNode;
//         meshkernel::UInt currentNode = edge.endNode;

//         currentNodes.push_back(prevNode);
//         edgeVisited[edge.id] = true;
...
//             const auto& options = boundaryAdjacency[currentNode];
//             double bestDelta = 3.0 * std::numbers::pi;
//             size_t chosenOptionIdx = static_cast<size_t>(-1);
...
//             firstTouchedFaces.push_back(associatedFaceId);
//         }
//     }
// }
...
void findBoundaryLoops(const std::vector<meshkernel::Point>& nodes,
                       const std::vector<Edge2>& edges,
                       std::vector<std::vector<meshkernel::UInt>>& allLoops,        // Out: The boundary node sequences
                       std::vector<std::vector<meshkernel::UInt>>& allTouchedFaces) // Out: All faces touched per loop in step order
{
    // Clear outputs to ensure clean state
    allLoops.clear();
    ...
    for (const auto& edge : edges)
    {
        if (edge.rightFace == constants::missing::uintValue)
        {
            ...
            // Note: edge.leftFace is the internal mesh face valid for both directions of boundary traversal
            boundaryAdjacency[edge.startNode].push_back({edge.id, edge.endNode, edge.leftFace, angleStartToEnd});
            boundaryAdjacency[edge.endNode].push_back({edge.id, edge.startNode, edge.leftFace, angleEndToStart});
        }
    }
    ...
    for (const auto& edge : edges)
    {
        ...
        std::vector<meshkernel::UInt> currentNodes;
        std::vector<meshkernel::UInt> currentFaces;

        meshkernel::UInt prevNode = edge.startNode;
        meshkernel::UInt currentNode = edge.endNode;

        currentNodes.push_back(prevNode);
        currentFaces.push_back(edge.leftFace); // First face touched by the initial edge
        edgeVisited[edge.id] = true;           // Mark edge as having been visited
        ...
        while (currentNode != edge.startNode)
        {
            ...
            const std::vector<BoundaryEdge>& boundaryEdges = boundaryAdjacency[currentNode];

            // The smallest visited angle.
            // All edge angles must be in interval [-2pi, 2pi],
            // So 3pi is used to indicate a suitable edge angle has been found
            double bestDelta = 3.0 * std::numbers::pi;
            meshkernel::UInt chosenEdgeIndex = constants::missing::uintValue;

            // Find the boundary edge that tracks closest clockwise to keep empty space on the right
            for (size_t i = 0; i < boundaryEdges.size(); ++i)
            {
                if (edgeVisited[boundaryEdges[i].edgeId])
                {
                    continue;
                }

                double delta = normalizeAngle(incomingAngle - boundaryEdges[i].angle);
            ...
            // So eigher the boundary loop was fully completed or a dead-end reached.
            ...
            prevNode = currentNode;
            currentNode = chosenEdge.neighbourNode;
...
// in edges
// in edgesFaces
// Out: The boundary node sequences
// Out: All faces touched per loop in step order
void findBoundaryLoops(const std::vector<meshkernel::Point>& nodes,
                       const std::vector<meshkernel::Edge>& edges,
                       const std::vector<std::array<UInt, 2>>& edgesFaces,
                       std::vector<std::vector<meshkernel::UInt>>& allLoops,
                       std::vector<std::vector<meshkernel::UInt>>& allTouchedFaces)
{
    allLoops.clear();
    allTouchedFaces.clear();
    ...
    // Collect all boundary edges and compute the angle
    for (meshkernel::UInt count = 0; count < edges.size(); ++count)
    // for (meshkernel::UInt count = 0; const auto& edge : edges)
    {
        // const meshkernel::Edge& edge = edges[count];
    ...
    for (meshkernel::UInt count = 0; count < edges.size(); ++count)
    {
        ...
        meshkernel::UInt prevNode = edges[count].first;
        meshkernel::UInt currentNode = edges[count].second;
        ...
        // Loop until we find the start node, making a closed boundary polygon
        while (currentNode != edges[count].first)
        {
            currentNodes.push_back(currentNode);
...
void findBoundaryLoops(const std::vector<meshkernel::Point>& nodes,
                       const std::vector<meshkernel::Edge>& edges,
                       const std::vector<std::array<UInt, 2>>& edgesFaces,
                       const meshkernel::Polygons& clippingPolygon,
                       std::vector<std::vector<meshkernel::UInt>>& allLoops,
                       std::vector<std::vector<meshkernel::UInt>>& allTouchedFaces)
{
    allLoops.clear();
    ...
    for (meshkernel::UInt count = 0; count < edges.size(); ++count)
    {
        ...
        if (currentNodes.size() >= 3)
        {
            std::vector<meshkernel::UInt> activeSegment;
            std::vector<meshkernel::UInt> activeFaces;

            for (size_t i = 0; i < currentNodes.size(); ++i)
            {

                // Check if this specific node falls inside the user's bounding box/polygon
                meshkernel::Point aNode = nodes[currentNodes[i]];
                auto [isIn, whichPoly] = clippingPolygon.IsPointInPolygons(aNode);

                if (isIn)
                {
                    activeSegment.push_back(currentNodes[i]);

                    if (i < currentFaces.size())
                    {
                        activeFaces.push_back(currentFaces[i]);
                    }
                }
                else
                {
                    // Node fell outside, push the collected segment if it holds meaningful data
                    if (activeSegment.size() >= 2)
                    {
                        std::cout << " activeSegment size: " << activeSegment.size() << std::endl;
                        activeSegment.push_back(activeSegment.front());
                        allLoops.push_back(activeSegment);
                        allTouchedFaces.push_back(activeFaces);
                    }
            ...
            // Push any remaining segment if the loop ended while still inside the polygon
            if (activeSegment.size() >= 2)
            {
                std::cout << " activeSegment size: " << activeSegment.size() << std::endl;
                activeSegment.push_back(activeSegment.front());
                // activeFaces.push_back(activeFaces.front());
                allLoops.push_back(std::move(activeSegment));
                allTouchedFaces.push_back(std::move(activeFaces));
            }
        }
    }
}

TEST(MeshRefinement, MeshWithHole_ShouldConstructMeshWithInteriorBoundaryPolygons)
{
    ...
    // Construct a mesh with holes
    meshkernel::Mesh2D mesh2(mesh.Edges(), mesh.Nodes(), mesh.m_facesNodes, mesh.m_numFacesNodes, mesh.m_projection);

    // Call administrate, to re-compute the face-node and face-edge connectivity.
    // This should not fill in the holes in the mesh
    mesh2.Administrate();

    meshkernel::SaveVtk(mesh2.Nodes(), mesh2.m_facesNodes, "mesh10.vtu");

    std::vector<Edge2> edges2(mesh2.Edges().size());

    for (meshkernel::UInt i = 0; i < mesh2.Edges().size(); ++i)
    {
        edges2[i] = Edge2{i, mesh2.Edges()[i].first, mesh2.Edges()[i].second, mesh2.m_edgesFaces[i][0], mesh2.m_edgesFaces[i][1]};
    }

    auto bLoops = findBoundaryLoops(mesh2.Nodes(), edges2, mesh2.m_edgesFaces);

    [[maybe_unused]] const auto& meshNodes = mesh2.Nodes();

    // for (size_t i = 0; i < bLoops.size(); ++i)
    for (const auto& loop : bLoops)
    {
        std::cout << "boudnary loop: ";

        for (size_t j = 0; j < loop.size(); ++j)
        {
            std::cout << "{" << loop[j].x << ", " << loop[j].y << "}, ";
        }
    ...
    std::vector<meshkernel::Point> clippingpolygonNodes{{-5.0, -5.0}, {140.0, 0.0}, {140.0, 35.0}, {-5.0, 35.0}, {-5.0, -5.0}};
    meshkernel::Polygons clippingPolygon(clippingpolygonNodes, mesh2.m_projection);

    findBoundaryLoops(mesh2.Nodes(), mesh2.Edges(), mesh2.m_edgesFaces, allLoops, allTouchedFaces);
    // findBoundaryLoops(mesh2.Nodes(), mesh2.Edges(), mesh2.m_edgesFaces, clippingPolygon, allLoops, allTouchedFaces);

    std::cout << std::endl;
    std::cout << "--------------------------------" << std::endl;

    for (meshkernel::UInt count = 0; const auto& loop : bLoops)
    {
        std::cout << "boudnary loop: " << "  " << count << "  " << loop.size() << ": " << ": ";
        // std::cout << "boudnary loop: " << allTouchedFaces[count].size() << "  " << loop.size() << ": " << allTouchedFaces[count][0] << ": ";

        auto boundaryPolygons = splitMultiplePolygons(loop);

        std::cout << "boudnary loop: " << loop.size() << "  " << boundaryPolygons.size() << "  ";
        ...
        for (size_t j = 0; j < boundaryPolygons.size(); ++j)
        {
            std::cout << " sub-loop " << boundaryPolygons[j].size() << "  ";

            for (size_t k = 0; k < boundaryPolygons[j].size(); ++k)
            {
                std::cout << "{" << boundaryPolygons[j][k].x << ", " << boundaryPolygons[j][k].y << "}, ";
            }
    ...
    for (meshkernel::UInt count = 0; const auto& loop : allLoops)
    {

        std::vector<meshkernel::Point> loopPnts(loop.size());

        for (size_t i = 0; i < loopPnts.size(); ++i)
        {
            loopPnts[i] = meshNodes[loop[i]];
        }
        ...
        auto [boundaryPolygons, firstTouchedFaces] = splitMultiplePolygons(loopPnts, allTouchedFaces[count]);

        std::cout << "boudnary loop: " << firstTouchedFaces.size() << "  " << loop.size() << "  " << boundaryPolygons.size() << "  ";

        for (size_t j = 0; j < loop.size(); ++j)
        {
            std::cout << "{" << loopPnts[j].x << ", " << loopPnts[j].y << "}, ";
        }

        std::cout << std::endl;
        std::cout << "face ids: ";

        for (size_t j = 0; j < allTouchedFaces[count].size(); ++j)
        {
            std::cout << allTouchedFaces[count][j] << ",  ";
        }
        ...
        for (size_t j = 0; j < boundaryPolygons.size(); ++j)
        {
            Polygons pgs(boundaryPolygons[j], mesh2.m_projection);
            std::cout << " sub-loop " << boundaryPolygons[j].size() << "  " << firstTouchedFaces[j] << "  " << std::boolalpha << pgs.IsPointInAnyPolygon(mesh2.m_facesMassCenters[firstTouchedFaces[j]]) << ": ";
    ...
    std::vector<Point> outerBoundaryPoints = mesh2.ComputeBoundaryPolygons(std::vector<meshkernel::Point>());

    std::cout << std::endl;
    std::cout << "--------------------------------" << std::endl;
    std::cout << "boudnary loop: ";

    for (size_t j = 0; j < outerBoundaryPoints.size(); ++j)
    {
        if (outerBoundaryPoints[j].IsValid())
        {
            std::cout << "{" << outerBoundaryPoints[j].x << ", " << outerBoundaryPoints[j].y << "}, ";
        }
        else
        {
            std::cout << std::endl;
    ...
    auto allBoundaryPoints = extractor.ExtractAll(mesh2);

    Polygons allBoundaryPolygons(allBoundaryPoints, mesh2.m_projection);

    for (UInt i = 0; i < allBoundaryPolygons.GetNumPolygons(); ++i)
    {
        const auto& enclosure = allBoundaryPolygons.Enclosure(i);
        const auto& enclosurePoints = enclosure.Outer().Nodes();

        std::cout << "enclosure points: ";

        for (size_t j = 0; j < enclosurePoints.size(); ++j)
        {
            std::cout << "{" << enclosurePoints[j].x << ", " << enclosurePoints[j].y << "}, ";
        }
