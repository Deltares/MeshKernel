...
TEST(MeshRefinement, MeshWithHole_ShouldGenerateInteriorBoundaryPolygons)
{
    auto curviMesh = MakeCurvilinearGrid(0.0, 0.0, 20.0, 20.0, 11, 11);
    const auto edges = curviMesh->ComputeEdges();
    const auto nodes = curviMesh->ComputeNodes();
    ...
    auto deleteMeshFacesUndoAction = mesh.DeleteMeshFaces2(boundaryWithMissingElement, true);
...
TEST(MeshRefinement, WTF)
{
    ...
    auto undoAction2 = mesh.DeleteMeshFaces2(boundaryWithMissingElement, true);
    ...
    [[maybe_unused]] auto undoAction3 = mesh.DeleteMeshFaces2(boundaryWithMissingElement2, false);
