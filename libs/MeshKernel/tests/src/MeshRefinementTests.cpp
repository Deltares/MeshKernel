...
TEST(MeshRefinement, CasulliRefinement)
{
    ...
    std::vector<Point> expectedPointVector;

    for (size_t i = 0; i < expectedPoints.size(); ++i)
    {
        expectedPointVector.push_back(mesh.Node(validNodeMap[i]));
    ...
    {
        es.push_back(mesh.GetEdge(validEdgeMap[i]).first);
        ee.push_back(mesh.GetEdge(validEdgeMap[i]).second);
