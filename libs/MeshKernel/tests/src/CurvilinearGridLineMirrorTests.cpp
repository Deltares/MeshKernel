...
TEST(CurvilinearLineMirror, Compute_LineMirrorOnUpperBoundary_ShouldAddFacesOnUpperBoundary)
{
    ...
    ASSERT_NEAR(81010.674000571220, mirroredGrid.m_gridNodes(2, last).x, tolerance);
    ASSERT_NEAR(81097.939900259138, mirroredGrid.m_gridNodes(3, last).x, tolerance);
    ASSERT_NEAR(81096.681464918671, mirroredGrid.m_gridNodes(4, last).x, tolerance);

    ASSERT_NEAR(367480.69596951915, mirroredGrid.m_gridNodes(0, last).y, tolerance);
    ASSERT_NEAR(367242.36746145069, mirroredGrid.m_gridNodes(1, last).y, tolerance);
    ASSERT_NEAR(367002.07900554762, mirroredGrid.m_gridNodes(2, last).y, tolerance);
    ASSERT_NEAR(366719.73488287395, mirroredGrid.m_gridNodes(3, last).y, tolerance);
    ASSERT_NEAR(366511.72792605805, mirroredGrid.m_gridNodes(4, last).y, tolerance);
}
...
TEST(CurvilinearLineMirror, Compute_LineMirrorOnRightBoundary_ShouldAddFacesOnRightBoundary)
{
    ...
    ASSERT_NEAR(273113.11689828755, mirroredGrid.m_gridNodes(last, 2).x, tolerance);
    ASSERT_NEAR(273421.82178223983, mirroredGrid.m_gridNodes(last, 3).x, tolerance);
    ASSERT_NEAR(273731.95226017234, mirroredGrid.m_gridNodes(last, 4).x, tolerance);
    ASSERT_NEAR(274115.35433400975, mirroredGrid.m_gridNodes(last, 5).x, tolerance);
    ASSERT_NEAR(274499.35511354846, mirroredGrid.m_gridNodes(last, 6).x, tolerance);
    ASSERT_NEAR(274883.99770888803, mirroredGrid.m_gridNodes(last, 7).x, tolerance);
    ASSERT_NEAR(275268.64743354439, mirroredGrid.m_gridNodes(last, 8).x, tolerance);

    ASSERT_NEAR(1246326.3868120508, mirroredGrid.m_gridNodes(last, 0).y, tolerance);
    ASSERT_NEAR(1246385.9365869928, mirroredGrid.m_gridNodes(last, 1).y, tolerance);
    ASSERT_NEAR(1246430.2519299081, mirroredGrid.m_gridNodes(last, 2).y, tolerance);
    ASSERT_NEAR(1246461.7442429555, mirroredGrid.m_gridNodes(last, 3).y, tolerance);
    ASSERT_NEAR(1246482.6999190229, mirroredGrid.m_gridNodes(last, 4).y, tolerance);
    ASSERT_NEAR(1246466.1900003948, mirroredGrid.m_gridNodes(last, 5).y, tolerance);
    ASSERT_NEAR(1246448.5627491081, mirroredGrid.m_gridNodes(last, 6).y, tolerance);
    ASSERT_NEAR(1246429.5894027406, mirroredGrid.m_gridNodes(last, 7).y, tolerance);
    ASSERT_NEAR(1246411.3593545749, mirroredGrid.m_gridNodes(last, 8).y, tolerance);
}
