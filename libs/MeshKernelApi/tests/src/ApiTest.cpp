...
TEST(Mesh2D, GetReducedData)
{
    const meshkernel::UInt numElementsX = 2;
    const meshkernel::UInt numElementsY = 2;
    const double delta = 1.0;

    // create mesh
    auto [numNodes, numEdges, nodesX, nodesY, edgeNodes] =
        MakeRectangularMeshForApiTesting(numElementsX,
                                         numElementsY,
                                         delta,
                                         meshkernel::Point(0.0, 0.0));
    meshkernelapi::Mesh2D initialMesh{};
    initialMesh.num_nodes = static_cast<int>(numNodes);
    initialMesh.num_edges = static_cast<int>(numEdges);
    initialMesh.node_x = nodesX.data();
    initialMesh.node_y = nodesY.data();
    initialMesh.edge_nodes = edgeNodes.data();
    ...
    // first initialise using the mesh, mesh2d
    errorCode = meshkernelapi::mkernel_mesh2d_set(mk_id, initialMesh);
    ASSERT_EQ(meshkernel::ExitCode::Success, errorCode);
    ...
    ASSERT_EQ(mesh2d.num_nodes, 9);
    ASSERT_EQ(mesh2d.num_edges, 12);
    ASSERT_EQ(mesh2d.num_faces, 4);
    ...
    std::vector<int> nodesPerFace(mesh2d.num_faces);
    std::vector<int> faceNodes(4 * mesh2d.num_faces);
    ...
    errorCode = meshkernelapi::mkernel_mesh2d_get_data_reduced(mk_id, mesh2d);
    ASSERT_EQ(meshkernel::ExitCode::Success, errorCode);

    // Check x-node values

    EXPECT_EQ(xNodes[0], 0.0);
    EXPECT_EQ(xNodes[1], 0.0);
    EXPECT_EQ(xNodes[2], 0.0);

    EXPECT_EQ(xNodes[3], delta);
    EXPECT_EQ(xNodes[4], delta);
    EXPECT_EQ(xNodes[5], delta);

    EXPECT_EQ(xNodes[6], 2.0 * delta);
    EXPECT_EQ(xNodes[7], 2.0 * delta);
    EXPECT_EQ(xNodes[8], 2.0 * delta);

    // Check y-node values

    EXPECT_EQ(yNodes[0], 0.0);
    EXPECT_EQ(yNodes[1], delta);
    EXPECT_EQ(yNodes[2], 2.0 * delta);

    EXPECT_EQ(yNodes[3], 0.0);
    EXPECT_EQ(yNodes[4], delta);
    EXPECT_EQ(yNodes[5], 2.0 * delta);

    EXPECT_EQ(yNodes[6], 0.0);
    EXPECT_EQ(yNodes[7], delta);
    EXPECT_EQ(yNodes[8], 2.0 * delta);

    // Check edge-node values
    ...
    EXPECT_EQ(edges[10], 5);
    EXPECT_EQ(edges[11], 8);

    EXPECT_EQ(edges[12], 1);
    EXPECT_EQ(edges[13], 0);

    EXPECT_EQ(edges[14], 2);
    EXPECT_EQ(edges[15], 1);

    EXPECT_EQ(edges[16], 4);
    EXPECT_EQ(edges[17], 3);

    EXPECT_EQ(edges[18], 5);
    EXPECT_EQ(edges[19], 4);

    EXPECT_EQ(edges[20], 7);
    EXPECT_EQ(edges[21], 6);

    EXPECT_EQ(edges[22], 8);
    EXPECT_EQ(edges[23], 7);

    // Check number of nodes per face

    EXPECT_EQ(nodesPerFace[0], 4);
    EXPECT_EQ(nodesPerFace[1], 4);
    EXPECT_EQ(nodesPerFace[2], 4);
    EXPECT_EQ(nodesPerFace[3], 4);

    // Check face node values

    EXPECT_EQ(faceNodes[0], 0);
    EXPECT_EQ(faceNodes[1], 3);
    EXPECT_EQ(faceNodes[2], 4);
    EXPECT_EQ(faceNodes[3], 1);

    EXPECT_EQ(faceNodes[4], 1);
    EXPECT_EQ(faceNodes[5], 4);
    EXPECT_EQ(faceNodes[6], 5);
    EXPECT_EQ(faceNodes[7], 2);

    EXPECT_EQ(faceNodes[8], 3);
    EXPECT_EQ(faceNodes[9], 6);
    EXPECT_EQ(faceNodes[10], 7);
    EXPECT_EQ(faceNodes[11], 4);

    EXPECT_EQ(faceNodes[12], 4);
    EXPECT_EQ(faceNodes[13], 7);
    EXPECT_EQ(faceNodes[14], 8);
    EXPECT_EQ(faceNodes[15], 5);
}

TEST(Mesh2D, GetReducedDataFailure)
{
    const meshkernel::UInt numElementsX = 2;
    const meshkernel::UInt numElementsY = 2;
    const double delta = 1.0;
    ...
    mesh2d.node_y = yNodes.data();
    mesh2d.edge_nodes = edges.data();
    mesh2d.nodes_per_face = nullptr;
    mesh2d.face_nodes = faceNodes.data();

    // nodes-per-face are null
    errorCode = meshkernelapi::mkernel_mesh2d_get_data_reduced(mk_id, mesh2d);
    ASSERT_EQ(meshkernel::ExitCode::MeshKernelErrorCode, errorCode);
    ...
    mesh2d.edge_nodes = edges.data();
    mesh2d.nodes_per_face = nodesPerFace.data();
    mesh2d.face_nodes = nullptr;

    // face-nodes are null
    errorCode = meshkernelapi::mkernel_mesh2d_get_data_reduced(mk_id, mesh2d);
    ASSERT_EQ(meshkernel::ExitCode::MeshKernelErrorCode, errorCode);

    // Now check node, edge and face sizes are correct
    ...
    // Reset number of nodes to correct value
    mesh2d.num_nodes = 9;
    ...
    // Reset number of edges to correct value
    mesh2d.num_edges = 12;
